...
- **오브젝트 보기 3원칙(거리)**: ① 말풍선은 오브젝트 위에만 — **루나 말풍선·독백 금지**(빌드 에러로 차단)
  ...
  정보도 서사도 없는 오브젝트는 배치하지 않는다(전봇대 예시). 외부 운영 지점 8개·외부 앵커 7개이며 sequential 실사용 0.
- **거리 데이터 범위(PD 확정)**: 외부 운영 지점은 기존 7개에 `p_elevator_move`를 더한 8개다. 완·노점·상점 데이터는 데모에서 제거했다. 삼호·시바 첫 만남과 삼호 사망·생존 결과는 거리 시스템에 넣지 않고 별도 연출 컷신으로 제작한다. 고양이는 `p_alley_cat.source_id=bubi`로 전신을 표시하며 플레이어가 E 상호작용한 뒤에만 대사를 시작한다. 엘리베이터 방송(`p_elevator_radio`)과 조작(`p_elevator_move`)은 별도 이벤트이며, 조작은 층 ID 없이 `system:elevator_toggle`이 현재 위치를 판단한다.
